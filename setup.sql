...
--     credenciales en cada llamada.
--   * La contraseña de ADMIN se guarda hasheada (bcrypt, irreversible) — protege
--     todo lo demás, no se puede recuperar, solo resetear.
--   * Las contraseñas de PROSPECTOS se guardan cifradas de forma REVERSIBLE
--     (pgcrypto pgp_sym_encrypt), a propósito: son tokens de acceso generados
--     por Quipu (no elegidos ni reusados por el prospecto) que solo destraban
--     información pública, y el admin necesita poder mostrárselas de nuevo si
--     el prospecto las pierde, sin tener que resetear cada vez. La clave de
--     cifrado (PASS_ENC_KEY, más abajo) vive solo en este archivo/base.
-- ============================================================
...
create table if not exists prospect_accounts (
  ...
  pass_enc text not null,                     -- contraseña cifrada reversible (ver PASS_ENC_KEY)
...
-- Devuelve la contraseña actual de una cuenta en texto plano (descifrada).
